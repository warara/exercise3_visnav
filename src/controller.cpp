  ...
  public:
    ...
    float getCommand(const ros::Time& t, float error, float derror)
    {
      ...
      float u=c_proportional*error+c_derivative*derror+c_integral*(e_int_incr+ diff.toSec()*error);
      t_old = t;
      return u;
    }
  ...
  public:
    ...
    void calculateContolCommand(const ros::Time& t)
    {
      ...
      float u_x=pid_x.getCommand(t, goal_x-state.x);
      float u_y=pid_y.getCommand(t, goal_y-state.y);
      float u_yaw=pid_yaw.getCommand(t, goal_yaw-state.yaw);
